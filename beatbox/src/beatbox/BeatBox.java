...
public class BeatBox {
	...
	private void buildTrackAndStart() {
		...
		for (int i = 0; i < 16; i++) {
			...
			for (int j = 0; j < 16; j++) {
				...
				if (jc.isSelected()) {
					...
					trackList.add(null);  // because this lot should be empty in the track
				}
			}
		...
		track.add(makeEvent(PROGRAM_CHANGE, 9, 1, 0, 15));  // -so we always go to16 beats
		try {
	...
	private void makeTracks(ArrayList<Integer> list) {
		for (int i = 0; i < list.size(); i++) {
			...
			if (instrumentKey != null) {
				...
				track.add(makeEvent(NOTE_OFF, 9, instrumentKey, 100, i));
			}
		}
	}
